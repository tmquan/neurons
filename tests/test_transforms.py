...
from neurons.transforms import ElasticDeformationd, MissingSectiond, Defectd
...
class TestDefectd:
    """Tests for Defectd transform."""

    def test_passthrough_when_prob_zero(self) -> None:
        t = Defectd(keys=("image",), prob=0.0)
        data = {"image": np.random.rand(32, 32).astype(np.float32)}
        result = t(data)
        np.testing.assert_array_equal(result["image"], data["image"])

    def test_output_clipped_to_01(self) -> None:
        t = Defectd(keys=("image",), prob=1.0, line_prob=0.5, intensity_prob=0.5)
        data = {"image": np.random.rand(32, 32).astype(np.float32)}
        result = t(data)
    ...
    def test_tensor_input_output(self) -> None:
        t = Defectd(keys=("image",), prob=1.0, line_prob=1.0, intensity_prob=0.0)
        data = {"image": torch.rand(32, 32)}
        result = t(data)
        assert isinstance(result["image"], torch.Tensor)
        assert result["image"].shape == (32, 32)
    ...
    def test_custom_probabilities(self) -> None:
        t = Defectd(keys=("image",), prob=0.5, line_prob=0.3, intensity_prob=0.7)
        assert t.prob == 0.5
