...
from neurons.transforms import ElasticDeformationd, MissingSectiond, Defectsd
...
class TestElasticDeformationd:
    """Tests for ElasticDeformationd transform."""

    def test_passthrough_when_prob_zero(self) -> None:
        t = ElasticDeformationd(keys=("image",), prob=0.0)
    ...
    def test_always_applies_when_prob_one(self) -> None:
        t = ElasticDeformationd(keys=("image",), prob=1.0, sigma=5.0, alpha=50.0)
    ...
    def test_multi_key_transform(self) -> None:
        t = ElasticDeformationd(keys=("image", "label"), prob=1.0, sigma=5.0, alpha=50.0)
        data = {
            "image": np.random.rand(32, 32).astype(np.float32),
    ...
    def test_custom_parameters(self) -> None:
        t = ElasticDeformationd(keys=("image",), sigma=20.0, alpha=200.0, prob=0.8)
        assert t.sigma == 20.0
...
class TestMissingSectiond:
    """Tests for MissingSectiond transform."""

    def test_passthrough_when_prob_zero(self) -> None:
        t = MissingSectiond(keys=("image",), prob=0.0)
        data = {"image": np.random.rand(10, 32, 32).astype(np.float32)}
        result = t(data)
        np.testing.assert_array_equal(result["image"], data["image"])

    def test_interpolate_mode(self) -> None:
        t = MissingSectiond(keys=("image",), prob=1.0, fill_mode="interpolate")
    ...
    def test_zero_mode(self) -> None:
        t = MissingSectiond(keys=("image",), prob=1.0, fill_mode="zero")
    ...
    def test_copy_mode(self) -> None:
        t = MissingSectiond(keys=("image",), prob=1.0, fill_mode="copy")
    ...
    def test_2d_input_not_modified(self) -> None:
        t = MissingSectiond(keys=("image",), prob=1.0)
        data = {"image": np.random.rand(8, 8).astype(np.float32)}
        result = t(data)
        np.testing.assert_array_equal(result["image"], data["image"])

    def test_multi_key(self) -> None:
        t = MissingSectiond(keys=("image", "label"), prob=1.0, fill_mode="zero")
        data = {
...
class TestDefectsd:
    """Tests for Defectsd transform."""

    def test_passthrough_when_prob_zero(self) -> None:
        t = Defectsd(keys=("image",), prob=0.0)
        data = {"image": np.random.rand(32, 32).astype(np.float32)}
        result = t(data)
        np.testing.assert_array_equal(result["image"], data["image"])

    def test_output_clipped_to_01(self) -> None:
        t = Defectsd(keys=("image",), prob=1.0, line_prob=0.5, intensity_prob=0.5)
        data = {"image": np.random.rand(32, 32).astype(np.float32)}
        result = t(data)
    ...
    def test_preserves_shape_2d(self) -> None:
        t = Defectsd(keys=("image",), prob=1.0)
        data = {"image": np.random.rand(32, 32).astype(np.float32)}
        result = t(data)
        assert result["image"].shape == (32, 32)
    ...
    def test_tensor_input_output(self) -> None:
        t = Defectsd(keys=("image",), prob=1.0, line_prob=1.0, intensity_prob=0.0)
        data = {"image": torch.rand(32, 32)}
        result = t(data)
        assert isinstance(result["image"], torch.Tensor)
        assert result["image"].shape == (32, 32)
    ...
    def test_custom_probabilities(self) -> None:
        t = Defectsd(keys=("image",), prob=0.5, line_prob=0.3, intensity_prob=0.7)
        assert t.prob == 0.5
