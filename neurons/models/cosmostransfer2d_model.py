"""
Cosmos-Transfer2.5 **2D** model wrapper for connectomics segmentation.

Adapts the Cosmos-Transfer2.5 Diffusion Transformer backbone (2B or 14B
...
_VARIANT_CONFIGS: Dict[str, _VariantConfig] = {
    ...
    "14B": _VariantConfig(
        hf_repo_id="nvidia/Cosmos-Transfer2.5-14B",
        hf_revision="diffusers/general",
        hidden_dim=5120,
        num_layers=40,
        num_heads=40,
...
def _download_from_hf(
    ...
    try:
        ...
        raise ImportError(
            "huggingface_hub is required for downloading Cosmos-Transfer2.5 "
            "weights.  Install with: pip install huggingface_hub"
        )

    cache_dir = cache_dir or str(Path.home() / ".cache" / "neurons" / "cosmos_transfer25")
...
class _DiTBlock(nn.Module):
    ...
    Simplified version of the Cosmos-Transfer2.5 DiT block for standalone
    use when the ``diffusers`` or ``cosmos_transfer2`` packages are not
...
class _StandaloneDiT(nn.Module):
    """Minimal Diffusion Transformer matching Cosmos-Transfer2.5 shape.
...
class _DecoderAdapter(nn.Module):
    """Reuses pretrained VAE decoder for multi-head segmentation output.
    ...
    def forward(
        ...
        if self._is_video_decoder:
            ...
            if decoded.dim() == 5:
                decoded = rearrange(decoded, "b c 1 h w -> b c h w")
...
class CosmosTransfer2DWrapper(nn.Module):
    """Cosmos-Transfer2.5 adapted for 2D connectomics segmentation.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.decoder_adapter = _DecoderAdapter(
        ...
        logger.info(
            "CosmosTransfer2DWrapper initialised: variant=%s, "
    ...
    def _try_load_diffusers(
        ...
        try:
            ...
            logger.info("Loaded 2-D backbone + VAE via diffusers (local snapshot).")
            return True
    ...
    def _try_load_cosmos_package(
        ...
        """Attempt to load via the ``cosmos_transfer2`` package."""
        try:
            # TODO: Exact import path depends on the installed cosmos_transfer2
            # version.  The package layout observed in the repo is:
            #   cosmos_transfer2._src.transfer2.models.dit
        ...
        try:
            ...
            if hasattr(pipe, "dit"):
                ...
                logger.warning("Could not find DiT module on cosmos_transfer2 pipeline.")
                return False

            self._backbone_loaded = True
            self._backend = "cosmos_transfer2"
            logger.info("Loaded backbone via cosmos_transfer2 package.")
            return True

        except Exception as exc:
            logger.debug("cosmos_transfer2 load failed: %s", exc)
            return False
    ...
    def _extract_features_hook(
        ...
        try:
            with torch.no_grad():
                ...
                null_condition = torch.zeros(B, 1, *latent_5d.shape[2:], device=latent.device, dtype=latent.dtype)
    ...
    def forward(
        ...
        out = self.decoder_adapter(features, target_size=(H_in, W_in))
...
def verify_fit(
    ...
    """Verify whether a Cosmos-Transfer2.5 variant fits the 3-stage task.
