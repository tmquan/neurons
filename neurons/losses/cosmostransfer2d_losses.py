"""
Cosmos-Transfer2.5 **2D** combined loss for image segmentation.
...
class CosmosTransfer2DLoss(nn.Module):
    """Composite loss for Cosmos-Transfer2.5 2D segmentation heads.

    Mirrors the :class:`Vista2DLoss` pattern but adds an optional
    ...
    Args:
        ...
        weight_edge: Boundary-pixel weight multiplier.
        weight_bone: Medial-axis pixel weight multiplier.
        delta_v: Pull margin.
        delta_d: Push margin.
    ...
    def _feature_consistency_loss(
        ...
        Both tensors should have shape ``[B, C, H, W]``.  The loss
        is the mean over all elements of the squared difference between
        L2-normalised feature vectors.
        """
        a_flat = rearrange(features_a, "b c h w -> b c (h w)")
        b_flat = rearrange(features_b, "b c h w -> b c (h w)")
