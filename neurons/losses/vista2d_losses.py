...
def _edt_worker(args):
    ...
    Args:
        args: tuple of (label_np_b, uid) — 2D label array and instance id.
...
class InstanceLoss(nn.Module):
    ...
    def _get_weight_boundary(self, label: torch.Tensor) -> torch.Tensor:
        ...
        label_4d = rearrange(label, "b ... -> b 1 ...")            # [B,1,H,W]
    ...
    def _skeleton_weight_torch(self, label: torch.Tensor) -> torch.Tensor:
        ...
        _CHUNK = 16
        max_iter = min(label.shape[-2:]) // 2 + 1
        ...
        for b in range(B):
            ...
            for start in range(0, K, _CHUNK):
                ...
                dt = dt / rearrange(max_d, "k c -> k c 1 1")
