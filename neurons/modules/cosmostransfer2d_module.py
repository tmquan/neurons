"""
Cosmos-Transfer2.5 **2D** Lightning Module for image segmentation training.

Supports two training modes (combinable in a single step):

- **automatic**: predict from the image alone.
...
variant can handle the 3-stage segmentation task within memory constraints.
"""
...
from neurons.models.cosmostransfer2d_model import (
    CosmosTransfer2DWrapper as _Model,
    verify_fit,
)
from neurons.losses.cosmostransfer2d_losses import CosmosTransfer2DLoss as _Loss
...
class CosmosTransfer2DModule(pl.LightningModule):
    """PyTorch Lightning module for 2-D Cosmos-Transfer2.5 segmentation.
    ...
    Args:
        model_config: Forwarded to :class:`CosmosTransfer2DWrapper`.
        optimizer_config: Optimizer / scheduler settings.
        loss_config: Forwarded to :class:`CosmosTransfer2DLoss`.
    ...
    def compatibility_check(
        ...
        """Verify the model fits the 3-stage segmentation task.
        ...
        **Stage mapping (2-D):**
        ...
        3 (Geometry)      DiT features -> geometry head  Backbone trained on natural
                                                        images; dir / cov heads
                                                        need fine-tuning.
        ================  ============================  ==========================
        """
        result = verify_fit(
        ...
        if self._variant == "14B" and self.criterion.weight_geometry > 0:
            result["warnings"].append(
                "14B + geometry head is memory-intensive.  Consider "
                "weight_geometry=0 or gradient checkpointing."
            )
