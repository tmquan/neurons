...
def _prepare_flat_labels(
    ...
    Returns:
        Tuple of (pred_flat, true_flat) numpy arrays.
        Returns empty arrays if no valid pixels.
    """
...
def compute_ari_point(
    pred_labels: torch.Tensor,
    true_labels: torch.Tensor,
    ignore_background: bool = True,
) -> float:
    """
    Compute Adjusted Rand Index (ARI) for a single sample.
    ...
    Returns:
        ARI value in [0, 1] (clamped from [-1, 1]).
    """
    try:
...
def compute_ami_point(
    pred_labels: torch.Tensor,
    true_labels: torch.Tensor,
    ignore_background: bool = True,
) -> float:
    """
    Compute Adjusted Mutual Information (AMI) for a single sample.
    ...
    Returns:
        AMI value in [0, 1] (clamped from [-1, 1]).
    """
    try:
...
def compute_axi_point(
    pred_labels: torch.Tensor,
    true_labels: torch.Tensor,
    ignore_background: bool = True,
) -> float:
    """
    Compute the geometric mean of ARI and AMI for a single sample.

    Provides a single combined score summarising both clustering metrics.
    ...
    Returns:
        AXI value in [0, 1].
    """
    ari = compute_ari_point(pred_labels, true_labels, ignore_background)
    ami = compute_ami_point(pred_labels, true_labels, ignore_background)
...
def compute_ari_batch(
    pred_labels: torch.Tensor,
    true_labels: torch.Tensor,
    ignore_background: bool = True,
) -> float:
    """
    Compute ARI averaged over a batch.
    ...
    Returns:
        Mean ARI over the batch.
    """
    total, count = 0.0, 0
    for b in range(pred_labels.shape[0]):
        v = compute_ari_point(pred_labels[b], true_labels[b], ignore_background)
        if v > 0:
            total += v
            count += 1
...
def compute_ami_batch(
    pred_labels: torch.Tensor,
    true_labels: torch.Tensor,
    ignore_background: bool = True,
) -> float:
    """
    Compute AMI averaged over a batch.
    ...
    Returns:
        Mean AMI over the batch.
    """
    total, count = 0.0, 0
    for b in range(pred_labels.shape[0]):
        v = compute_ami_point(pred_labels[b], true_labels[b], ignore_background)
        if v > 0:
            total += v
            count += 1
    return total / count if count > 0 else 0.0
...
def compute_axi_batch(
    pred_labels: torch.Tensor,
    true_labels: torch.Tensor,
    ignore_background: bool = True,
) -> float:
    """
    Compute AXI (geometric mean of ARI and AMI) averaged over a batch.
    ...
    Returns:
        Mean AXI over the batch.
    """
    total, count = 0.0, 0
    for b in range(pred_labels.shape[0]):
        v = compute_axi_point(pred_labels[b], true_labels[b], ignore_background)
        if v > 0:
            total += v
            count += 1
    return total / count if count > 0 else 0.0
